...
const Pokedex = () => {
    ...
    try {
      ...
      if (type) {
        ...
        response = await fetch(
          `/api/pokemon/type/${type}?limit=${limit + 1}&offset=${offsetValue}&sort=${sortOrderValue}&searchTerm=${query}`,
          {
        ...
        response = await fetch(
          `/api/pokemon/search/${query}?limit=${limit + 1}&offset=${offsetValue}&sort=${sortOrderValue}&type=${type}`,
          {
        ...
        response = await fetch(
          `/api/pokemon?limit=${limit + 1}&offset=${offsetValue}&sort=${sortOrderValue}&shuffle=${shouldShuffle}`
        )
      }
      setIsLoading(false)
      if (response.ok) {
  ...
  return (
    <div>
      ...
      {morePokemon && !isFetching && (
        <LoadMoreButton isLoading={isLoading} onLoadMore={loadMorePokemon} />
      )}
    </div>
  )
}
